...
for i = 1:m
    ...
    y_i = (tmp == y(i)); % 1*10
    h_x_i = h_x(:, i); % 10*1
    J = J + y_i * log(h_x_i) + (1 - y_i) * log(1 - h_x_i);

    % grad calc
    delta3 = h_x_i - y_i'; % 10*1
    Theta2_grad = Theta2_grad + delta3 * a_2(:,i)'; % 10*26

    g_z_2 = [1; sigmoidGradient(z_2(:, i))]; % 26*1
    delta2 = Theta2' * delta3 .* g_z_2; % 26*1
    delta2 = delta2(2:end); % 25*1
    Theta1_grad = Theta1_grad + delta2 * a_1(i, :); % 25*401
endfor
J = J * -1 / m;
